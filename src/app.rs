...
LEFT JOIN
    valuation v
ON
    v.id IN (SELECT id FROM (SELECT id, MAX(at) FROM valuation v2 WHERE v2.asset_id = a.id GROUP BY v2.id))
GROUP BY
    p.name"
;
    ...
    pub async fn portfolio(&self, args: PortfolioArgs) {
        match args.cmd {
            ...
            PortfolioCommands::Show { name } => {
                if name == "all" {
                    let rows = sqlx::query_as::<_, PortfolioAllRow>(QUERY_PORTFOLIO_SHOW_ALL)
                        .fetch_all(&self.pool)
                        .await
                        .unwrap();
